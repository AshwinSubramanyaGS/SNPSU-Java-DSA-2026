...
public class Main {
    ...
    public static void main(String[] args) {
        runTest(1, () -> Solution.subsets(new int[]{1,2,3}), 8, x -> x instanceof List && ((List<?>)x).size() == 8);
        runTest(2, () -> Solution.subsets(new int[]{4,9}), 4, x -> x instanceof List && ((List<?>)x).size() == 4);
        runTest(3, () -> Solution.subsets(new int[]{}), 1, x -> x instanceof List && ((List<?>)x).size() == 1); // EDGE CASE
